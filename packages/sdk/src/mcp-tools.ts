...
export function createAgentHubMcpTools(client: AgentHubControlClient): AgentHubMcpTool[] {
  return [
    ...
    {
      name: "agent_hub_observe_ops_status",
      description: "Run repeated operational status snapshots for deployment observation windows.",
      inputSchema: {
        project: z.string().optional(),
      ...
      handler: async (args) => toMcpText(await client.observeOpsStatus(compactDefined({
        project: stringArg(args.project),
        iterations: numberArg(args.iterations),
        intervalMs: numberArg(args.intervalMs),
        alertLimit: numberArg(args.alertLimit),
        executionLimit: numberArg(args.executionLimit),
        failOnWarning: booleanArg(args.failOnWarning),
      }) as AgentHubObserveOpsStatusOptions)),
    },
    {
      name: "agent_hub_get_recovery_plan",
      description: "Generate backup, upgrade, rollback, and verification commands for Agent Hub operations.",
      inputSchema: {
        project: z.string().optional(),
        ...
        databaseUrlConfigured: z.boolean().optional(),
        executionLimit: z.number().int().min(1).optional(),
      },
      handler: async (args) => toMcpText(client.getRecoveryPlan(compactDefined({
        project: stringArg(args.project),
        backupDir: stringArg(args.backupDir),
        backupFile: stringArg(args.backupFile),
        serviceName: stringArg(args.serviceName),
        envFile: stringArg(args.envFile),
        databaseUrlConfigured: booleanArg(args.databaseUrlConfigured),
        executionLimit: numberArg(args.executionLimit),
      }) as AgentHubRecoveryPlanOptions)),
    },
    {
      name: "agent_hub_get_recovery_drill_plan",
      description: "Generate a safe backup and disposable restore rehearsal plan for Agent Hub operations.",
      inputSchema: {
        project: z.string().optional(),
        backupDir: z.string().optional(),
        backupFile: z.string().optional(),
        envFile: z.string().optional(),
        restoreDatabaseEnvVar: z.string().optional(),
        databaseUrlConfigured: z.boolean().optional(),
        restoreDatabaseUrlConfigured: z.boolean().optional(),
        executionLimit: z.number().int().min(1).optional(),
      },
      handler: async (args) => toMcpText(client.getRecoveryDrillPlan(compactDefined({
        project: stringArg(args.project),
        backupDir: stringArg(args.backupDir),
        backupFile: stringArg(args.backupFile),
        envFile: stringArg(args.envFile),
        restoreDatabaseEnvVar: stringArg(args.restoreDatabaseEnvVar),
        databaseUrlConfigured: booleanArg(args.databaseUrlConfigured),
        restoreDatabaseUrlConfigured: booleanArg(args.restoreDatabaseUrlConfigured),
        executionLimit: numberArg(args.executionLimit),
      }) as AgentHubRecoveryDrillPlanOptions)),
    },
    {
      name: "agent_hub_run_recovery_drill",
      description: "Execute a guarded Agent Hub backup and disposable restore rehearsal. Requires explicit restore database reset confirmation.",
      inputSchema: {
        project: z.string().optional(),
        backupDir: z.string().optional(),
        backupFile: z.string().optional(),
        ...
        restoreDatabaseUrlConfigured: z.boolean().optional(),
        executionLimit: z.number().int().min(1).optional(),
        commandTimeoutMs: z.number().int().min(1).optional(),
        confirmRestoreDatabaseReset: z.boolean().optional(),
      },
      handler: async (args) => toMcpText(await client.runRecoveryDrill(compactDefined({
        project: stringArg(args.project),
        backupDir: stringArg(args.backupDir),
        backupFile: stringArg(args.backupFile),
        envFile: stringArg(args.envFile),
        ...
        restoreDatabaseUrlConfigured: booleanArg(args.restoreDatabaseUrlConfigured),
        executionLimit: numberArg(args.executionLimit),
        commandTimeoutMs: numberArg(args.commandTimeoutMs),
        confirmRestoreDatabaseReset: booleanArg(args.confirmRestoreDatabaseReset),
      }) as AgentHubRunRecoveryDrillOptions)),
    },
    {
      name: "agent_hub_run_release_check",
      description: "Run the Agent Hub release gate across doctor, ops status, optional recovery drill, optional canary, and observation.",
      inputSchema: {
        project: z.string().optional(),
        alertLimit: z.number().int().min(1).optional(),
        executionLimit: z.number().int().min(1).optional(),
        failOnWarning: z.boolean().optional(),
        includeRecoveryDrill: z.boolean().optional(),
        confirmRestoreDatabaseReset: z.boolean().optional(),
        backupDir: z.string().optional(),
        backupFile: z.string().optional(),
        envFile: z.string().optional(),
        ...
        restoreDatabaseUrlConfigured: z.boolean().optional(),
        commandTimeoutMs: z.number().int().min(1).optional(),
        canaryAgent: z.string().optional(),
        canaryPayload: z.record(z.string(), z.unknown()).optional(),
        canaryTimeoutMs: z.number().int().min(1).optional(),
        canaryIntervalMs: z.number().int().min(0).optional(),
        observeIterations: z.number().int().min(1).optional(),
        observeIntervalMs: z.number().int().min(0).optional(),
        skipObserve: z.boolean().optional(),
      },
      handler: async (args) => toMcpText(await client.runReleaseCheck(compactDefined({
        project: stringArg(args.project),
        alertLimit: numberArg(args.alertLimit),
        executionLimit: numberArg(args.executionLimit),
        failOnWarning: booleanArg(args.failOnWarning),
        includeRecoveryDrill: booleanArg(args.includeRecoveryDrill),
        confirmRestoreDatabaseReset: booleanArg(args.confirmRestoreDatabaseReset),
        backupDir: stringArg(args.backupDir),
        backupFile: stringArg(args.backupFile),
        envFile: stringArg(args.envFile),
        ...
        restoreDatabaseUrlConfigured: booleanArg(args.restoreDatabaseUrlConfigured),
        commandTimeoutMs: numberArg(args.commandTimeoutMs),
        canaryAgent: stringArg(args.canaryAgent),
        canaryPayload: recordArg(args.canaryPayload),
        canaryTimeoutMs: numberArg(args.canaryTimeoutMs),
        canaryIntervalMs: numberArg(args.canaryIntervalMs),
        observeIterations: numberArg(args.observeIterations),
        observeIntervalMs: numberArg(args.observeIntervalMs),
        skipObserve: booleanArg(args.skipObserve),
      }) as AgentHubReleaseCheckOptions)),
    },
    {
